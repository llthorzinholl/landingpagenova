...
const FloatingAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Welcome to AES. I can assist you with information regarding asbestos removal, soil remediation, and chemical decontamination. How can I help today?' }
  ]);
    ...
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
          ...
          <div className="bg-aes-navy p-6 text-white">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="bg-aes-cyan w-10 h-10 rounded flex items-center justify-center text-sm font-black">AES</div>
                <div>
                  <h4 className="font-bold text-sm tracking-tight uppercase">Technical Assistant</h4>
                  <div className="flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                    <span className="text-[10px] text-white/60 tracking-widest uppercase">Expert System</span>
                  </div>
                </div>
              </div>
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-5 bg-slate-50">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-4 py-3 rounded ${
                  msg.role === 'user' 
              ...
              <div className="flex justify-start">
                <div className="bg-white border border-slate-200 px-4 py-3 rounded">
                  <div className="flex gap-1.5">
                    <div className="w-1.5 h-1.5 bg-aes-cyan rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                    <div className="w-1.5 h-1.5 bg-aes-cyan rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                    <div className="w-1.5 h-1.5 bg-aes-cyan rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                  </div>
                </div>
              </div>
            )}
      ...
      >
        <div className="flex flex-col items-start leading-none uppercase">
          <span className="text-[10px] tracking-[0.2em] font-black opacity-70 mb-1">Hazmat AI</span>
          <span className="font-bold text-xs">{isOpen ? 'Close System' : 'Open Assistant'}</span>
        </div>
